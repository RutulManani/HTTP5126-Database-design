...
--  A 
ALTER TABLE product
ADD CONSTRAINT c_price CHECK (price >= 0);
--  B 
UPDATE order_product
SET product_id = (SELECT product_id FROM product WHERE name = 'Canon Camera')
WHERE order_id = (SELECT order_id FROM `order` WHERE email = 'jane@gmail.com')
AND product_id = (SELECT product_id FROM product WHERE name = 'Apple Smartphone');
